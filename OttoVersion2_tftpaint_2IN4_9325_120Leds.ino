...
#include <Wire.h>
// #include <LM75.h>
// sensor;  // initialize an LM75 object (temp sensor), not used in this project
...
int PIR_PIN = 3;                // choose the input pin (for PIR sensor.
int pirState = LOW;             // we start, assuming no motion detected.
int val = 0;                    // variable for reading the pin status.

// Define LDR parameters
const int LDR_PIN = A5;  // Analog input.
int LightpinValue = 0;  // The current read value.
...
String ButtonText[ARRAYSIZE] = { "OFF", "Scanner", "Cylon","PACMAN", "Police", "Theatre","Fire", "Auto"};
...
// #############################################################################################################################
    ...
    tft.println("Version 2.0 RGB-V");
    tft.println("120 LED Version with PIR sensor");
    tft.println("By Otto M. Klaasen (C) 2019");
...
MyLed = 0;
Forward = 1;
xs = (NUM_LEDS/2)-10; // Start Value
ys = (NUM_LEDS/2)+10; // Max Value
...
//Serial.println(" C");
TempSensor = 35;
// Fan script not used, not enough pins.
// If the LEDS are OFF then set FAN to off (0).
//if (currentRGBscript = 0 or currentcolor == BLACK){
//  analogWrite(FAN,0);
//}
//else
//{  
...
// Script6 Fire
       ...
       if ((currentRGBscript == 7 & SelectColorButton == 0 & SelectColorBar == 0) && WriteColor != BLACK ) {
             ...
             // Read the LDR.
             LightpinValue = analogRead(LDR_PIN);
             //Serial.println(LightpinValue);
  
             val = digitalRead(PIR_PIN);  // read input value of PIR sensor
             // Serial.println(val);
             //Serial.println(pirState);      
                   
                        if (val == HIGH and LightpinValue < 800) {            // check if the input is HIGH and its dark.
                          // digitalWrite(ledPin, HIGH);  // turn LED ON, Not used and not defined
                          if (pirState == LOW) {
                            ...
                            pirState = HIGH;
                            // Set LED bar on
                            for (int j=0; j < NUM_LEDS; j++) leds[j] = rgb16convertor(WHITE);
                            FastLED.show();
                          }
                        } else {
                          // digitalWrite(ledPin, LOW); // turn LED OFF, Not used and not defined
                          if (pirState == HIGH){
